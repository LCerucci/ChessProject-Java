...
public class ChessMatch {
	...
	public ChessPiece replacePromotedPiece(String type) {
		if(promoted == null) {
			throw new ChessException("This piece can't be promoted.");
		}
	...
	private Piece makeMove(Position source, Position target) {
		...
		if(p instanceof King && target.getColumn() == source.getColumn() + 2) {
			...
			Position targetR = new Position(target.getRow(), target.getColumn() + 1);
	...
	private void validateSourcePosition(Position position) {
		if(!board.thereIsAPiece(position)) {
			throw new ChessException("There's no piece in the source.");
		}
		if(currentPlayer != ((ChessPiece)board.piece(position)).getColor()) {
			throw new ChessException("This piece isen't yours.");
		}
	...
	private void validateTargetPosition(Position source, Position target) {
		if(!board.piece(source).isThereAnyPossibleMove()) {
			throw new ChessException("This peace can't move to this target.");
		}
	}
	...
	private ChessPiece king(Color color) {
		...
		throw new IllegalStateException("There is no king of such color.");
